...
$sql_exp=mysqli_query($conn,"SELECT SUM(expenditure) AS 'expenditure',name,date,phone FROM ((SELECT Teacher_name AS 'name',date AS 'date',SUM(actual_fees) AS 'expenditure',Teacher_phone AS 'phone' FROM `teacher_fees` INNER JOIN `teacher` ON `teacher_fees`.`teacher_id` = `teacher`.`Teacher_id` WHERE `date` LIKE '$curr_year-$curr_month-%' GROUP BY date) UNION (SELECT payment_name,payment_date,SUM(payment_amt),payment_phone FROM `expenditure`  WHERE `payment_date` LIKE '$curr_year-$curr_month-%' GROUP BY payment_date)) AS A GROUP BY date ORDER BY `date` DESC");

$sql_income=mysqli_query($conn,"SELECT SUM(money) AS 'money',name,date,phone FROM ((SELECT Student_name AS 'name',SUM(Actual_fees) AS 'money',Phone_no AS 'phone', date AS 'date' , `fees_history`.month AS 'month' FROM `fees_history` INNER JOIN `subject_master` ON `fees_history`.`subject_id` = `subject_master`.`Subject_id` INNER JOIN student_registration ON `student_registration`.`Student_id`=`fees_history`.`student_id` INNER JOIN student_activity ON `student_activity`.`Subject_id` = `fees_history`.`subject_id` AND `student_activity`.`Student_id`=`fees_history`.`student_id` WHERE `fees_history`.`date` LIKE '$curr_year-$curr_month-%' GROUP BY date ) UNION (SELECT payment_by,SUM(payment_amt),payment_phone,payment_date,payment_month FROM `income` WHERE `payment_date` LIKE '$curr_year-$curr_month-%' GROUP BY payment_date)) AS A GROUP BY date ORDER BY date DESC;");

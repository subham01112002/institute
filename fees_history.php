...
<select onchange="month(this.value)">
  ...
  <table>
    ...
    <tbody class="scroll-pane">
        ...
        <tr>
            ...
            $paid=0;
            $id=$arr['id'];
            $query=mysqli_query($conn,"SELECT * FROM `fees_history` WHERE `student_id`='$id' AND `month` LIKE '%$curr_month'");
            $lat_date=mysqli_fetch_array(mysqli_query($conn,"SELECT MAX(date) AS 'date' FROM `fees_history` WHERE `student_id`='$id' AND `month` LIKE '%$curr_month'"));
            if($lat_date)
            {
            ...
            ?>
        <td  id="id-<?php echo $i ?>" data-id="<?php echo $arr['id']; ?>"><?php echo $arr['Student_name'] ?></td>
        ...
        while($sub2=mysqli_fetch_array($subject)){ ?>  
              <input type="checkbox" class="fees-<?php echo $i ?>"  data-sub='<?php echo $sub2['Subject_id'] ?>' value="<?php echo $sub2['Actual_fees'] ?>" onclick="fees(this,<?php echo $i ?>)" <?php if(in_array($sub2['Subject_id'] ,$arr_it)) { echo "checked"; $paid+=$sub2['Actual_fees']; } ?> > <?php echo  $sub2['Actual_fees']; ?><br/>
             <?php   }?>
                </td>
...
<script>
    function submit(num)
    {
        ...
        window.location.href="fees_sub.php?sub="+str+"&date="+date+"&id="+id;

    }
    function month(val){
        window.location.href="fees_history.php?month="+val;
    }
